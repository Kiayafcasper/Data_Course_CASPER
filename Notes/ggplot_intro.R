...
=======
p <- ggplot(midwest) # what do you see?

# give it some aesthetics to work with...
names(midwest)
ggplot(midwest, mapping = aes(x=area, y=poptotal))  # area and poptotal are columns in 'midwest'

             # aesthetics = aes x an y are two culonms in "modwest" 
             # now its needs to know what to draw
...
ggplot(midwest, aes(x=area, y=poptotal)) + # + means another layer
  geom_point() # The "+" tells ggplot to add another layer to our base plot

# Add another geom ... a trendline:
        #geom blank specifices the sspecs of the line
p <- ggplot(midwest, aes(x=area, y=poptotal)) + geom_point(colour='purple', size = 4, shape = 5) + 
  geom_smooth(method = "lm", colour = "magenta", formula = y~log(x), fill = "orange")
        # lm = linear model
        # chang color? can copy hex code from google color!
?geom_smooth
p
>>>>>>> c57341f (Assignment 3)
...
=======
p2 <- p + coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) # how is this different?
?lims.   # lims gets rid of data points outside of range
?coord_cartesian.   # kees whole data set
>>>>>>> c57341f (Assignment 3)
...
=======
        #aes mut be a colums name

# Don't like those colors?
p3 + scale_color_brewer(palette = "Set1")
p3+ scale_colour_viridis_d(option = 'mako', end = 0.8) + theme_light()
>>>>>>> c57341f (Assignment 3)
...
=======
p4 + facet_wrap(~ state, scales = "free") + theme(legend.position = "none", strip.text.x = element_text(size = 12, face="bold"))
>>>>>>> c57341f (Assignment 3)
